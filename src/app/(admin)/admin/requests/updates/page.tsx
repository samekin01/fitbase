...
import { updateUpdateRequestStatus } from "@/lib/actions/requests";

export const dynamic = "force-dynamic";
export const metadata = { title: "情報修正依頼 | FitBase CMS" };
...
export default async function UpdateRequestsPage() {
  ...
  const { data: requests } = await supabase
    .from("gym_update_requests")
    .select("id, status, created_at, gyms(name), name, email, content")
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    <RequestStatusSelect
                      ...
                      action={updateUpdateRequestStatus.bind(null, r.id)}
                    />
                  </td>
                </tr>
              ))}
