  ...
  return (
    <form action={formAction}>
      ...
      <div style={{ backgroundColor: "var(--color-white)", border: "1px solid var(--color-gray-200)", borderRadius: "var(--radius-md)", padding: "1.25rem", marginBottom: "1rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          <Field label="SEOタイトル">
            <input name="seo_title" type="text" defaultValue={article?.seo_title ?? ""} className="form-input" />
          </Field>
          <Field label="メタディスクリプション">
            <textarea name="meta_description" rows={2} defaultValue={article?.meta_description ?? ""} className="form-input" style={{ resize: "vertical" }} />
          </Field>
          <Field label="カノニカルURL">
            <input name="canonical_url" type="url" defaultValue={article?.canonical_url ?? ""} className="form-input" />
          </Field>
          <Field label="noindex">
            <CheckField name="noindex" label="検索エンジンにインデックスさせない" defaultChecked={article?.noindex} />
          </Field>
        </div>
      </div>
